...
void Mat4::addBox(Vec4 const &v, Vec4 const &dims)
{
    ...
    addEdge({x1, y1, z}, {x1, y1, z1});

    addEdge({x, y, z1}, {x1, y, z1});
    addEdge({x, y, z1}, {x, y1, z1});
    addEdge({x1, y, z1}, {x1, y1, z1});
    addEdge({x, y1, z1}, {x1, y1, z1});
}
    ...
    for (int phiStep = 0; phiStep < phiSteps; phiStep++)
    {
        for (int thetaStep = 0; thetaStep < thetaSteps; thetaStep++)
        {
            for (int i = 0; i < 8; i += 2)
            {
                ...
                Vec4 point = {r * std::cos(phi) + v.getX(),
    ...
    for (int phiStep = 0; phiStep < phiSteps; phiStep++)
    {
        for (int thetaStep = 0; thetaStep < thetaSteps; thetaStep++)
        {
            for (int i = 0; i < 8; i += 2)
            {
                double phi = phiStep * phiStepSize;
                double theta = thetaStep * thetaStepSize;
                Vec4 point = {(r2 + r1 * std::cos(phi)) * std::cos(theta) + v.getX(),
